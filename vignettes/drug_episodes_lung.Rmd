---
title: "Constructing drug episodes for lung (cisplatin|carboplatin)"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Constructing drug episodes for lung (cisplatin|carboplatin)}
...
```{r echo = FALSE}
fg_data <- transitions %>%
  dplyr::filter(from_state == "On_Treatment_Target_Line") %>%
  dplyr::mutate(
    fstatus = dplyr::case_when(
      to_state == "Death" ~ 1,
      to_state %in% c("progression", "off_treatment") ~ 2,
      TRUE ~ 0  # censored
    ),
    ftime = time_to_transition,
    group = chemo_line
  )


# Model matrix for group if it's not numeric already
fg_data <- fg_data %>%
  dplyr::filter(!is.na(fstatus), !is.na(ftime), ftime > 0)

# Fine-Gray model
fg_model <- crr(
  ftime = fg_data$ftime,
  fstatus = fg_data$fstatus,
  cov1 = model.matrix(~ factor(fg_data$group))[,-1]  # reference is line 1
)
